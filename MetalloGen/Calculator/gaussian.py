...
class Gaussian:
    ...
    def __init__(self, command="g16", working_directory=None):
        ...
        # 使用 -f 静音删除失败（如 Gau-* 不存在时不再在日志里刷 warning）
        self.rm_command = '/bin/rm -f '
    ...
    def execute(self,file_name,save_chk_file=None):
        # 将 Gaussian 的标准输出/错误重定向到 log 文件，避免在批量运行时刷屏
        os.system(f"{self.command} {file_name}.com > {file_name}.log 2>&1")
    ...
    def make_input(
        ...
        **kwargs
        ...
        # 核数：环境变量 GAUSS_NPROCSHARED > 调用方 params > 默认 8
        if "%nprocshared" not in params:
            nproc = os.environ.get("GAUSS_NPROCSHARED", "8")
            params["%nprocshared"] = str(nproc)
    ...
    def run(
        ...
        **kwargs
        ...
        # 静默 cclib 在解析截断日志时的错误输出，只通过返回值判断成败
